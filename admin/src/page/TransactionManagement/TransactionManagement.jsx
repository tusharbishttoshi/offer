...
export default function TransactionManagement() {
  ...
  const AllTransactionListApi = async () => {
    setLoader(true);
    try {
      const response = await axios.post(`/api/v1/transactions/list`, {
        // id: astro._id,
        limit: pageSize || 10,
        // type: selectedType,
        fromDate: startDate,
                ...
                <tbody style={{ overflow: "scroll" }}>
                  {Transaction?.map((user, index) => (
                    <tr key={user?._id}>
                      ...
                      <td className="fs-7" > {user?.userPaid <= 0 && user?.id} </td>
                      ...
                      <td className="fs-7" > {user?.astro_name} </td>
                      ...
                      <td className="fs-7"  style={{color:user?.astroEarn >= 0 ?"#5CFF5C":"#f06445"}}> {user?.astroEarn && "$" } {user?.astroEarn?.toFixed(2)}</td>
                      <td className="fs-7"  style={{color:user?.userPaid >= 0 ?"#5CFF5C":"#f06445"}}> {user?.userPaid && "$" } {user?.userPaid?.toFixed(2)}</td>
                      <td className="fs-7" >
                        {user?.time}
                      </td>
                      <td className="fs-7" >
