...
export default function SubAdmin() {
  ...
  const AllAstrologerListApi = async () => {
    setLoader(true);
    try {
      const response = await axios.post(`/api/v1/sub-admin/list`, {
        search: Search,
        limit: pageSize,
      ...
      <div className="row  m-2">
        ...
        <div className="col-md-2 text-start">
            <Link to="/Create-Subadmin">
              <button type="button" className="btn btn-primary btn text-black">
                Create 
              </button>
            </Link>
          </div>
                ...
                <tbody>
                  {AllTransition?.map((item, i) => (
                    <tr key={i}>
                      ...
                      <td className="fs-7"> {item?.createdAt.split("T").slice(0, 1)} ,{" "}
                        {new Date(item?.createdAt).getHours()}:
                        {new Date(item?.createdAt).getMinutes()}</td>
